...
const certs = [
  { title: 'Cloud Foundations', issuer: 'AWS', color: '#ff9900', date: 'Mar 2025' },
  { title: 'Cloud Practitioner Essentials', issuer: 'AWS', color: '#ff9900', date: 'Jun 2025' },
  { title: 'Cloud Operations', issuer: 'AWS', color: '#ff9900', date: '2025' },
  { title: 'Cloud Architecture', issuer: 'AWS', color: '#ff9900', date: '2025' },
]

const achievements = [
  { title: 'Secretary', org: 'UPES ACM', color: '#00ff88' },
  { title: 'Head of Events', org: 'UPES ACM-W', color: '#8b5cf6' },
  { title: 'Campus Ambassador', org: 'UnStop', color: '#00d4ff' },
  { title: 'Runner-Up', org: 'IBM ICE DAY', color: '#f59e0b' },
  { title: '150+ DSA Problems', org: 'LeetCode — 50 & 100 Days Badges', color: '#f97316' },
  { title: '15+ Technical Events', org: '2400+ Participants', color: '#34d399' },
  { title: 'Letter of Recommendation', org: 'Innoventory Solutions', color: '#00ff88' },
  { title: 'Letter of Recommendation', org: 'Roots2Bloom NGO', color: '#34d399' },
]
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            {certs.map((cert, i) => (
              <motion.div key={i}
                ...
                whileHover={{ y: -5 }}
                style={{ background: 'var(--bg-card)', border: `1px solid ${cert.color}33`, borderRadius: '10px', padding: '22px', transition: 'box-shadow 0.3s' }}
                onMouseEnter={e => e.currentTarget.style.boxShadow = `0 8px 30px ${cert.color}20`}
                onMouseLeave={e => e.currentTarget.style.boxShadow = 'none'}>
                <p style={{ fontWeight: 600, marginBottom: '4px' }}>{cert.title}</p>
                <p style={{ color: cert.color, fontFamily: 'Fira Code', fontSize: '0.78rem' }}>{cert.issuer}</p>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.75rem', marginTop: '4px' }}>{cert.date}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            {achievements.map((ach, i) => (
              <motion.div key={i}
                ...
                whileHover={{ y: -5 }}
                style={{ background: 'var(--bg-card)', border: `1px solid ${ach.color}33`, borderRadius: '10px', padding: '22px', transition: 'box-shadow 0.3s' }}
